...
var (
	...
	ITypedEventHandlerDataTransferManagerDataRequestedEventArgsFunc func(_ uintptr, _ uintptr, args *IDataRequestedEventArgs)
)
...
func NewTypedEventHandler(guid *ole.GUID, callback uintptr) *ITypedEventHandler {
		...
		VTBL: &ITypedEventHandlerVTBL{
			IUnknownVtbl: ole.IUnknownVtbl{
				QueryInterface: syscall.NewCallback(func(source, idp unsafe.Pointer, result *unsafe.Pointer) uintptr {
					id := (*ole.GUID)(idp)
					if ole.IsEqualGUID(id, guid) || ole.IsEqualGUID(id, ole.IID_IUnknown) {
						*result = source
						return ole.S_OK
					}
					return 0x80004002 // ENOTINTERFACE
				}),
